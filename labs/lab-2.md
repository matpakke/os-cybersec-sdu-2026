...
>
> ```
> ตอบ:fork() จริงๆ แล้วเรียก syscall ชื่อ: clone()
>2. -f หมายถึง: Follow forks (บอกให้ strace ติดตามไปดูข้อมูลของ Child process ที่ถูกสร้างขึ้นมาใหม่ด้วย)
>คำอธิบายเพิ่มเติม (วิเคราะห์จากภาพ)
>ทำไมถึงเป็น clone ไม่ใช่ fork?
>ใน Linux ยุคใหม่ ฟังก์ชัน fork() ใน Library จะถูกเปลี่ยนให้มาเรียกใช้ clone() system call เพียงตัวเดียว เพราะ clone มีความยืดหยุ่นสูงกว่า (สามารถกำหนดได้ว่าจะแชร์หน่วยความจำหรือไฟล์ร่วมกันไหม ซึ่งเป็นพื้นฐานของการสร้างทั้ง Process และ Thread ครับ)
>สังเกตในภาพบรรทัดที่ 2: มีการเรียก clone(...) และคืนค่าเป็น 45591 (นั่นคือ PID ของลูกนั่นเอง)
>
>ความสำคัญของ Option -f (Follow forks):
>ถ้าคุณไม่ใส่ -f ตัว strace จะรายงานเฉพาะสิ่งที่ Parent (PID 45590) ทำเท่านั้น คุณจะไม่เห็นบรรทัดที่ขึ้นต้นด้วย [pid 45591] เลย ซึ่งหมายความว่าคุณจะมองไม่เห็นตอนที่ลูกเรียก execve เพื่อไปรันคำสั่ง date ครับ
>
>เกร็ดน่ารู้จาก Log:
>สังเกตตรงที่มีการเรียก execve ซ้ำๆ หลายครั้ง (ที่เป็น -1 ENOENT) นั่นคือการทำงานของ execlp (ตัว p คือ PATH) ระบบกำลังไล่หาไฟล์ date ในโฟลเดอร์ต่างๆ ตามลำดับ:
>
>หาใน /home/kali/.local/bin/ (ไม่เจอ)
>
>หาใน /usr/local/sbin/ (ไม่เจอ)
>
>... จนกระทั่งเจอที่ /usr/bin/date ในบรรทัดที่ค่าส่งกลับเป็น 0 (สำเร็จ!)
>
>
> ```
